...
@section('content')
    <div class="card">
        <div class="card-header">
            <p class="float-left">Edit book</p>
            <a href="{{ route('backend_book_manage') }}" class="btn btn-sm btn-success float-right"><i class="fa fa-sellsy"></i>  Manage Book</a>
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('backend_book_update',$book->id) }}" enctype="multipart/form-data" data-parsley-validate class="bg-light shadow p-3">
                @csrf
{{ method_field('PUT') }}
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" pattern="^[A-Za-z _-]+$" value="{{ $book->name }}" required autocomplete="name" autofocus>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        <select name="book_category_id" class="form-control @error('book_category_id') is-invalid @enderror">
                            @if($book->book_category_id <= 0)
                                <option value="0">Nothing select building</option>
                            @else
                            <option value="{{ $book->book_category->id }}">{{ $book->book_category->name }}</option>
                            @endif
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        <select name="class_id" class="form-control @error('class_id') is-invalid @enderror">
                            ...
                            @else
                            <option value="{{ $book->class->id }}">{{ $book->class->name }}</option>
                            @endif
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        <input id="author_name" type="text" class="form-control @error('author_name') is-invalid @enderror" name="author_name" pattern="^[A-Za-z - _]+$" value="{{ $book->author_name }}" required  autocomplete="author_name" autofocus>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-8">
                        <input id="publication" type="text" class="form-control @error('publication') is-invalid @enderror" name="publication" value="{{ $book->publication }}"  required pattern="^[A-Za-z - _]+$" autocomplete="publication">
                ...
                <div class="form-group row mb-0">
                    <div class="col-md-8 offset-md-3">
                        <button type="submit" class="btn btn-sm btn-success">
                            <i class="fa fa-upload"></i>  Update book
                        </button>
                    </div>
                </div>
            </form>
